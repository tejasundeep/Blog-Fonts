...
//# My Code Start
var app=angular.module('digitalDownloads',[]);app.controller('crudController',function($scope,$http){$scope.buttonName="Add";$scope.change_page=function(num){if(num<=$scope.total_pages&&num!==0){$scope.current_page=0;$scope.start_record=0;$scope.start_record_show=0;$scope.total_records=$scope.original_items.length;$scope.page_size=5;$scope.end_record=0;$scope.total_pages=Math.ceil($scope.total_records/5);if($scope.total_pages>0){$scope.current_page=num;$scope.start_record=(num-1)*$scope.page_size;$scope.start_record_show=$scope.start_record+1;$scope.end_record=($scope.current_page==$scope.total_pages)?$scope.total_records:(num*$scope.page_size)}$scope.items=$scope.original_items.slice($scope.start_record,$scope.end_record)}};$scope.displayUsers=function(){$http.get('select.php').success(function(data){$scope.current_page=0;$scope.total_pages=1;$scope.original_items=data;$scope.change_page(1)})};$scope.insertData=function(){if($scope.name===null||$scope.category===null||$scope.price===null||$scope.preview===null||$scope.download===null){$scope.message_color="red";$scope.message="All fields are required."}else{$http.post('insert.php',{'name':$scope.name,'category':$scope.category,'price':$scope.price,'preview':$scope.preview,'download':$scope.download,'buttonName':$scope.buttonName,'id':$scope.id}).success(function(){$scope.message_color="green";$scope.message="Success.";$scope.name=null;$scope.category=null;$scope.price=null;$scope.preview=null;$scope.download=null;$scope.buttonName="Add";$scope.displayUsers()}).error(function(){console.log("Error")})}}});var loadDeferredStyles=function(){var addStylesNode=document.getElementById("deferred-styles");var replacement=document.createElement("div");replacement.innerHTML=addStylesNode.textContent;document.body.appendChild(replacement);addStylesNode.parentElement.removeChild(addStylesNode)};var raf=window.requestAnimationFrame||window.mozRequestAnimationFrame||window.webkitRequestAnimationFrame||window.msRequestAnimationFrame;if(raf)raf(function(){window.setTimeout(loadDeferredStyles,0)});else window.addEventListener('load',loadDeferredStyles);
